...
void printTable()
{
    ...
    while (print != NULL)
    {
        ...
        if (print->dimenstions != NULL)
        {
            printf_s(" [ ");
            while (print->dimenstions != NULL)
            {
                printf_s(" %s, ", print->dimenstions->dim);
                print->dimenstions = print->dimenstions->next;
            }
            printf_s(" ] ");
        }
        printf_s(" %s\n", print->role);
...
void parse_FUNC_PROTOTYPE() //FUNC_PROTOTYPE -> RETURNED_TYPE id ( PARAMS )
{
    ...
    case COMMA_TOKEN: {fprintf(yyout, "parse_FUNC_PROTOTYPE - COMMA_TOKEN\n"); break; }
...
void parse_COMP_STMT()
{
    ...
    if (table->father != NULL) //we're moving up to check predefs
    {
        checkPredef(table->father, entry);
    }
}
...
void parse_ARGS()
{
    ...
    case ID_TOKEN: {fprintf(yyout, "parse_ARGS - ID_TOKEN\n"); break; }
    case INT_NUM_TOKEN: {fprintf(yyout, "parse_ARGS - INT_NUM_TOKEN\n"); break; }
    case FLOAT_NUM_TOKEN: {fprintf(yyout, "parse_ARGS - FLOAT_NUM_TOKEN\n"); break; }
...
void parse_FACTOR()//FACTOR -> id FACTOR_TAG | int_num | float_num | ( EXPR )
{
    ...
    case LEFT_PARENTHESES_TOKEN: {fprintf(yyout, "RULE(FACTOR -> ( EXPR ) )\n");
        ...
        match(RIGHT_PARENTHESES_TOKEN);
        break; }
    case FLOAT_NUM_TOKEN: {fprintf(yyout, "RULE(FACTOR -> FLOAT_NUM)\n"); break; }
    case INT_NUM_TOKEN: {fprintf(yyout, "RULE(FACTOR -> INT_NUM)\n"); break; }
